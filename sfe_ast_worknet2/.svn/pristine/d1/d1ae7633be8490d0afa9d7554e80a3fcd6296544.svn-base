...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		<script>
			jQuery(document).ready(function() 
			{
				...
				jQuery('.search_file').click(function() {
					jQuery('#openPdfUploader').click();
				});
												
				jQuery('#openPdfUploader').change(function() {
					var pdfCount = 0;
					var imgCount = 0;
					...
					if(jQuery(this).val() != '') {
						...
						for(var i=0 ; i < this.files.length; i++) {
							...
							if (ext == 'pdf') {
								...
								alert('PDF 또는 이미지 파일만 업로드 할 수 있습니다.');
								jQuery(this).val('');
								return;
							}						
								
							if (pdfCount > 0 && imgCount > 0) {
								alert('PDF 나 이미지 파일 한 종류만 업로드 할 수 있습니다.');
								jQuery(this).val('');
								return;
							}
							...
							if (pdfCount > 1)	
							{
								alert('PDF 파일은 하나만 업로드 할 수 있습니다.');
								jQuery(this).val('');
								return;
							}
							
							addHtml += '<li class="file">'+this.files[i].name+'</li>';
						}
						
						if (pdfCount > 0) {
							jQuery('#fileType').val('PDF');
						} else if (imgCount > 0) {
							jQuery('#fileType').val('IMG');
						} else {
							jQuery('#fileType').val('');
						}
						
						jQuery('form ul').append(addHtml);
					}
				});
			})
	...
	<body>
		<form:form id="fileUploadForm" name="fileUpload" method="POST" action="pdfUpload.do" enctype="multipart/form-data" modelAttribute="uploadForm">
			<ul>
			...
			<input id="openPdfUploader" style="display:none" type="file" name="files" value='' multiple>
		</form:form>
	</body>
</html>