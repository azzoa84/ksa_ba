 ...
 * <h3>Thread Safety</h3>
 * The {@link ImageUtil} class is <strong>thread-safe</strong> (as all the methods
...
public class ImageUtil {
    ...
    if (DEBUG)
      t = System.currentTimeMillis();
		...
		if (scalingMethod == ImageUtil.Method.AUTOMATIC)
		...
		if (scalingMethod == ImageUtil.Method.SPEED) {
			result = scaleImage(src, targetWidth, targetHeight,
					RenderingHints.VALUE_INTERPOLATION_NEAREST_NEIGHBOR);
		} else if (scalingMethod == ImageUtil.Method.BALANCED) {
			result = scaleImage(src, targetWidth, targetHeight,
					RenderingHints.VALUE_INTERPOLATION_BILINEAR);
		} else if (scalingMethod == ImageUtil.Method.QUALITY
				|| scalingMethod == ImageUtil.Method.ULTRA_QUALITY) {
			/*
	 ...
	 * {@link ImageUtil#DEBUG} is <code>true</code>; otherwise the VM will be
	 ...
	 * 
	 * @see ImageUtil#LOG_PREFIX
	 * @see ImageUtil#LOG_PREFIX_PROPERTY_NAME
	 */
	protected static void log(int depth, String message, Object... params) {
		if (ImageUtil.DEBUG) {
			System.out.print(ImageUtil.LOG_PREFIX);
	 ...
	 * method utilizes the {@link ImageUtil#THRESHOLD_QUALITY_BALANCED} and
	 * {@link ImageUtil#THRESHOLD_BALANCED_SPEED} thresholds when selecting which
	...
	protected static Method determineScalingMethod(int targetWidth,
		...
		if (length <= ImageUtil.THRESHOLD_QUALITY_BALANCED)
			result = Method.QUALITY;
		else if (length <= ImageUtil.THRESHOLD_BALANCED_SPEED)
